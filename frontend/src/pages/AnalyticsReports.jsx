...
function buildAnalyticsExportRows(report) {
  ...
  addCsvSection(rows, "Report Metadata", ["Field", "Value"], [
    ...
    ["Analytics Mode", report.analyticsMode || "observed"],
  ], (item) => item);
  ...
  addCsvSection(rows, "5. Operational Performance Metrics", ["Measure", "Value"], [
    ["Occupancy Utilization", formatPercent(metrics.occupancyPct)],
  ...
  addCsvSection(rows, "6. Data Quality & Recognition Reliability", ["Metric", "Value"], [
  ...
  addCsvSection(rows, "7. Detailed Statistical Appendix", ["Statistic", "Value"], [
...
function buildPdfMarkup(report, chartCards = []) {
  ...
  return `
    ...
    <div class="analytics-export-root">
      ...
      <div class="analytics-export-page">
        <div class="section-title">5. Operational Performance Metrics</div>
        <div class="callout-list">
          ${recommendations.map((line) => `<div class="callout-item">${escapeHtml(line)}</div>`).join("")}
        </div>
        <div class="section-title">6. Data Quality & Recognition Reliability</div>
        ${renderSimpleTable(["Metric", "Value"], dataQualityRows)}
        ${pageFooter}
      </div>

      <div class="analytics-export-page">
        <div class="section-title">7. Detailed Statistical Appendix</div>
...
function AnalyticsReportsInner() {
  ...
  async function runAnalyticsPipeline({ silent = false, force = false } = {}) {
    ...
    try {
      const [analyticsResult, realtimeResult] = await Promise.allSettled([
        fetchJson("/api/analytics-reports?mode=observed"),
        fetchJson("/api/dashboard?filter=today"),
      ]);

      let analyticsPayload;
      let analyticsMode = "observed";
  ...
  const recommendations = [
    ...
    `Use the observed ${fmt(last30Total)} visits from the current 30-day window as the planning baseline.`,
    reliabilityScore < 88
      ...
      <section className="section">
            ...
            >
              <div>
                ...
                >
                  ...
                  <span>{currentData?.analytics_mode === "basic" ? "Basic analytics mode" : "Observed analytics mode"}</span>
                </div>
              </div>
        ...
        >
          ...
          <InsightChip
            ...
            body={`Use ${fmt(last30Total)} observed visits from the current 30-day window for service scheduling.`}
          />
          <InsightChip
        ...
        <div style={{ marginBottom: 10, fontSize: 12, textTransform: "uppercase", letterSpacing: "0.07em", fontWeight: 800, color: ANALYTICS_COLORS.muted }}>
          6. Data Quality & System Reliability
        </div>
        <div
          style={{
            display: "grid",
        ...
        >
          ...
          >
            <span style={{ fontSize: 13, textTransform: "uppercase", letterSpacing: "0.06em", fontWeight: 800, color: ANALYTICS_COLORS.muted }}>
              7. Detailed Technical Analytics
            </span>
            <span style={{ fontSize: 12, color: ANALYTICS_COLORS.muted }}>
              Expand for statistical diagnostics
            </span>
          </summary>
