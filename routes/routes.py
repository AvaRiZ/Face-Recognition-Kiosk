...
def create_routes_blueprint(deps):
    ...
    def api_analytics_reports():
        ...
        library_open_hour = 7
        library_close_hour = 19

        removed_low_conf = 0
        removed_outside_hours = 0
        removed_duplicates = 0
        after_conf = []

        for row in all_raw:
            _, sr_code, name, program, gender, year_level, confidence, timestamp_value, source = row
            if source == "live":
                conf_value = _coerce_confidence(confidence)
                if conf_value is None or conf_value < 0.50:
                    removed_low_conf += 1
                    continue
            else:
                conf_value = float(confidence or 0.85)

            after_conf.append(
                {
                    "sr_code": (sr_code or "").strip(),
                    "name": (name or "").strip(),
        ...
        for row in after_conf:
            ...
            if len(timestamp_value) == 10:
                timestamp_value = f"{timestamp_value} 08:00:00"

            try:
                hour = int(timestamp_value[11:13])
            except (TypeError, ValueError, IndexError):
                removed_outside_hours += 1
                continue

            if hour < library_open_hour or hour >= library_close_hour:
                removed_outside_hours += 1
                continue

            row["timestamp"] = timestamp_value
            after_hours.append(row)

        seen_per_day = set()
        cleaned_rows = []
        for row in sorted(after_hours, key=lambda item: (item["timestamp"], item["sr_code"])):
            day_key = (row["timestamp"] or "")[:10]
            sr_code = row["sr_code"]
            if not sr_code or not day_key:
                removed_duplicates += 1
                continue

            dedupe_key = (sr_code, day_key)
        ...
        data_quality = {
            ...
            "removed_outside_hrs": removed_outside_hours,
        ...
        daily_counts_map = {}
        student_visit_days = {}
        gender_visits = {"Male": 0, "Female": 0, "Other": 0, "Unknown": 0}
        year_level_visits = {}
        ...
        for row in cleaned_rows:
            ...
            student_entry = student_visit_days.setdefault(
                row["sr_code"],
                {
                    "name": row["name"] or "-",
                    "sr_code": row["sr_code"],
            ...
            student_entry["days"].add(day)
            if row["name"] and student_entry["name"] == "-":
                student_entry["name"] = row["name"]
            if row["program"] and student_entry["program"] == "-":
                student_entry["program"] = row["program"]
            if row["year_level"] and student_entry["year_level"] == "-":
                student_entry["year_level"] = row["year_level"]
            if row["gender"] and not student_entry["gender"]:
                student_entry["gender"] = row["gender"]

            normalized_gender = (row["gender"] or "").strip().title()
            if normalized_gender in {"Male", "Female", "Other"}:
                gender_visits[normalized_gender] += 1
            else:
                gender_visits["Unknown"] += 1

            if row["year_level"]:
                year_level_visits[row["year_level"]] = year_level_visits.get(row["year_level"], 0) + 1

        sorted_days = sorted(daily_counts_map.items())
        daily_counts_list = [count for _, count in sorted_days]
        if daily_counts_list:
            mean_visits = round(sum(daily_counts_list) / len(daily_counts_list), 1)
            sorted_counts = sorted(daily_counts_list)
            count_n = len(sorted_counts)
            if count_n % 2:
                median_visits = sorted_counts[count_n // 2]
            else:
                median_visits = round((sorted_counts[(count_n // 2) - 1] + sorted_counts[count_n // 2]) / 2, 1)
            max_visits = max(daily_counts_list)
            min_visits = min(daily_counts_list)
            variance = sum((value - mean_visits) ** 2 for value in daily_counts_list) / count_n
        ...
        descriptive_stats = {
            "mean_daily_visits": mean_visits,
            "median_daily_visits": median_visits,
            "max_daily_visits": max_visits,
            "min_daily_visits": min_visits,
            "std_dev": std_dev,
        ...
        dow_sums = [0] * 7
        dow_counts = [0] * 7
        for day_str, count in sorted_days:
            try:
                weekday = date.fromisoformat(day_str).weekday()
            except ValueError:
                continue
            dow_sums[weekday] += count
            dow_counts[weekday] += 1
        dow_averages = [
            round(dow_sums[index] / dow_counts[index], 1) if dow_counts[index] else 0
        ...
        for offset in range(30):
            day_value = (start_30d + timedelta(days=offset)).isoformat()
            last_30_labels.append(day_value[5:])
            last_30_counts.append(daily_counts_map.get(day_value, 0))

        gender_data = [{"label": label, "count": count} for label, count in gender_visits.items() if count > 0]
        year_level_data = sorted(
            [{"label": label, "count": count} for label, count in year_level_visits.items()],
        ...
        for offset in range(27, -1, -1):
            day_value = (today - timedelta(days=offset)).isoformat()
            last_28_counts.append(daily_counts_map.get(day_value, 0))

        if any(last_28_counts) and mean_visits > 0:
            recent_7 = last_28_counts[-7:]
            weights = [1, 2, 3, 4, 5, 6, 7]
            weighted_base = sum(value * weight for value, weight in zip(recent_7, weights)) / sum(weights)
            forecast_variance = sum((value - weighted_base) ** 2 for value in recent_7) / len(recent_7)
            forecast_std = math.sqrt(forecast_variance)

            for day_offset in range(1, 8):
                future_date = today + timedelta(days=day_offset)
                weekday = future_date.weekday()
                dow_factor = (dow_averages[weekday] / mean_visits) if mean_visits else 1.0
                dow_factor = max(0.3, min(2.0, dow_factor))
                predicted = round(weighted_base * dow_factor)
                lower_bound = max(0, round(predicted - forecast_std))
                upper_bound = round(predicted + forecast_std)

                forecast_labels.append(future_date.strftime("%a %m/%d"))
        ...
        forecast = {
            ...
            "method": "7-day weighted moving average with day-of-week adjustment",
        }

        if sorted_days:
            first_day = date.fromisoformat(sorted_days[0][0])
            last_day = date.fromisoformat(sorted_days[-1][0])
            total_weeks = max(1, math.ceil(((last_day - first_day).days + 1) / 7))
        else:
            total_weeks = 1
        ...
        occasional = []
        rare = []
        for _, info in student_visit_days.items():
            total_visits = len(info["days"])
            weekly_avg = round(total_visits / total_weeks, 2)
            student = {
                "name": info["name"],
                "sr_code": info["sr_code"],
            ...
            if weekly_avg >= 3:
                ...
                occasional.append(student)
            else:
                rare.append(student)

        regular.sort(key=lambda item: item["total_visits"], reverse=True)
        occasional.sort(key=lambda item: item["total_visits"], reverse=True)
        rare.sort(key=lambda item: item["total_visits"], reverse=True)
        ...
        if std_dev > 0:
            for day_str, count in sorted_days:
                z_score = (count - mean_visits) / std_dev
                if abs(z_score) >= 2.0:
                    try:
                        dow_name = dow_labels[date.fromisoformat(day_str).weekday()]
                    except ValueError:
                        dow_name = ""
                    anomalies.append(
                        {
                            ...
                            "deviation": (
                                f"{abs(round(z_score, 1))}x std dev "
                                f"{'above' if z_score > 0 else 'below'} average"
                            ),
                        }
                    )
