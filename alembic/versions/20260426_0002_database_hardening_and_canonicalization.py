...
def upgrade() -> None:
    ...
    if _column_type(bind, "programs", "program_code") is None:
        op.execute("ALTER TABLE programs ADD COLUMN program_code TEXT")
...
def downgrade() -> None:
    ...
    # Drop program_code column if it exists (reverse of upgrade)
    if _column_type(bind, "programs", "program_code") is not None:
        op.execute("ALTER TABLE programs DROP COLUMN program_code")
